...
**Not in this report:** per-host results. No run of this index has published host names, and the
earlier per-host files still in this repository use positional identifiers — see "Why we are not
publishing the host list" below.
...
No run of this index has named individual hosts, and neither will the ones after it. The earlier
per-host result files are still in this repository, and their host column carries positional
identifiers, not host names. We are not going to remove them.
